    ...
    <body class="nav-md" onload="setInterval('reloadwaktu()');">
                      ...
                      <form class="form-horizontal form-label-left" action="brng_keluar.php" method="post">
                        ...
                        <div class="item form-group">
                          ...
                          <div class="col-md-3 col-sm-3 col-xs-12">
                            <select class="form-control" name="inpidsup" onchange="namadistributor(this.value);" required>
                              <option>- ID distributor -</option>
                              <?php
                              // ambil data dari database untuk ID distributor
                              $sql      = "SELECT * FROM dta_distributor";
                              $distributor = $mysqli->query($sql);
                              $jsarray  = "var idsup = new Array();";
                              // menampilkan data
                              while ($row = $distributor->fetch_array(MYSQLI_ASSOC)) {
                              ?>
                                <option value="<?php echo $row['id_distributor'] ?>"><?php echo $row['id_distributor']; ?></option>
                              <?php
                              // menampilkan data hasil dipilih
                              $jsarray .= "idsup['".$row['id_distributor']."'] = {namasup: '".addslashes($row['nama'])."'};";
                                }
                               ?>
                            </select>
                          </div>
                          <div class="col-md-3 col-sm-3 col-xs-12">
                            <input id="setdistributor" class="form-control col-md-7 col-xs-12" type="text" name="inpnmasup" placeholder="Nama distributor" readonly>
                          </div>
                        </div>
      ...
      <?php
       ...
       if (isset($_POST['keluar'])) {
         ...
         if ($row = $q_idtrns->fetch_row()) {
           ...
           $query  = "INSERT INTO dta_trnsaksi_brng_keluar (id_transaksi, id_distributor, nmasup, kd_barang, nmabar,
      ...
      // menampilkan nama distributor
      <?php echo $jsarray; ?>
      function namadistributor(iddistributor) {
        document.getElementById('setdistributor').value = idsup[iddistributor].namasup;
      }
